...
| [`proc-watchdog/proc_watchdog.rb`](proc-watchdog/) | Linux | Process watchdog built directly on `/proc`: per-pattern CPU/RSS/uptime plus crash-loop detection via a PID state file (RUNNING/RESTARTED/MISSING). |
| [`scheduled-task-audit/scheduled_task_audit.rb`](scheduled-task-audit/) | Windows | Security audit of Windows Scheduled Tasks via the Task Scheduler COM API: flags SYSTEM tasks running from user-writable dirs, missing binaries, hidden and stale tasks. |
...
| [`kernel-module-audit/module_audit.rb`](kernel-module-audit/) | Linux | Audits loaded kernel modules from `/proc/modules`, `/sys/module/*` and `/proc/sys/kernel/tainted` — attributes kernel taint to specific modules, flags CIS-blacklisted modules, and diffs against an approved baseline. A `--root` switch audits captured snapshots. |
| [`win-autorun-audit/autorun_audit.rb`](win-autorun-audit/) | Windows | Inventories and risk-scores every autorun entry — `Run`/`RunOnce` keys in both 32- and 64-bit registry views, Startup folders, and WMI `Win32_StartupCommand` — then diffs against a saved baseline. Ships a fixture-driven `--self-test` that runs off-Windows. |
...
| [`defender-status-audit/defender_status_audit.rb`](defender-status-audit) | Windows | Audits Microsoft Defender health via Get-MpComputerStatus (WMI) — real-time protection, signature age, tamper protection — with a stub test harness. |
